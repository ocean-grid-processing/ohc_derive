...
`apply` runs the named prescription, dumps the footprint png and a `coverage_<tag>_<window>_<level>_<mask>.nc`
...
def apply(name, level, constituents, reference_bathy, out_dir=".", require_top=None, tag=None, window=None):
    ...
    _dump_png(footprint, reference_bathy, level.name, name, out_dir, tag, window)
    _dump_coverage(footprint, height, reference_bathy, level, name, out_dir, tag, window)
...
def _dump_coverage(footprint, height, reference_bathy, level, mask_name, out_dir, tag=None, window=None):
    ...
    tokens = [t for t in (tag, window) if t]                          # tag/window optional; level+mask always
...
def _dump_png(footprint, reference_bathy, level_name, mask_name, out_dir, tag=None, window=None):
    try:
        import matplotlib
        matplotlib.use("Agg")
